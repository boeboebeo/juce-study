...
//생성자
GainEffectorAudioProcessorEditor::GainEffectorAudioProcessorEditor (GainEffectorAudioProcessor& p)
	: AudioProcessorEditor (&p), audioProcessor (p) //초기화 리스트
{
	...
	gainAttachment = std::make_unique<SliderAttachment> (
		audioProcessor.apvts, GainEffectorAudioProcessor::GAIN_DB_ID, gainSlider);

	phaseInvertAttachment = std::make_unique<ButtonAttachment> (
		audioProcessor.apvts, GainEffectorAudioProcessor::PHASE_INVERT_ID, phaseInvertButton);
